"""
FailGuard Protected Agent v8 — LangGraph + Grok + Audit Database
================================================================

Changes from v7:
  1. Async audit database wired in (failguard_db.py).
     Every pre_check and post_check evaluation is logged to
     data/failguard_audit.db asynchronously — zero latency impact
     on the evaluation pipeline. Each record includes the full
     evaluation result, plain English audit summary, and
     applicable regulatory flags.

  2. CSV filename prefix updated to results_v8_*.

  Unchanged: supervisor v7, reranker v6, context builder,
  graph topology, Grok system prompt, vote_threshold=1, top_k=3,
  TAXONOMY_PATH, MODEL_NAME, reranker_q6 threading,
  route_post_check fix.

  Database location: data/failguard_audit.db
  Miss Analyzer CLI: python src/supervisor/failguard_analyzer.py --help
"""
...
if __name__ == "__main__":
    print(f"FailGuard Protected Agent v8 — {len(test_cases)}-prompt evaluation\n")
    ...
    csv_path   = os.path.join(LOG_DIR, f"results_v8_{timestamp}.csv")
    fieldnames = [
    ...
    print(f"\nv8 evaluation complete. {len(test_cases)} prompts processed.")
