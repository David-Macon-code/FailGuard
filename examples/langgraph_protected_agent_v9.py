"""
FailGuard Protected Agent v9 — LangGraph + Grok + Auto Miss Detection
=====================================================================

Changes from v8:
  1. Feedback Footprint auto-detection wired in.
     When a batch run produces a wrong verdict (label known), the system
     automatically classifies the root cause, suggests a fix, and writes
     to feedback_footprint in the audit database — no manual --import-csv
     step required. The Miss Analyzer CLI is fully populated after every run.

  2. CSV filename prefix updated to results_v9_*.

  Unchanged: supervisor v7, reranker v6, context builder, graph topology,
  Grok system prompt, vote_threshold=1, top_k=3, audit DB async logging,
  route_post_check fix.
...
def pre_check_node(state: AgentState) -> dict:
    ...
    # Async audit log — fires in background, never blocks
    db.write_async(
        result=result,
        prompt_text=user_prompt,
        agent_response_text="",
        check_type="pre_check",
        environment="test" if "test_prompts" in __file__ else "production",
    )
...
if __name__ == "__main__":
    print(f"FailGuard Protected Agent v9 — {len(test_cases)}-prompt evaluation\n")
    ...
    for idx, (user_input, label) in enumerate(test_cases, 1):
        ...
        # ---- AUTO MISS DETECTION ----
        # If verdict is wrong and we have the ground truth label,
        # automatically classify root cause and log to feedback_footprint.
        # Uses a small delay to let the async DB write complete first.
        if not correct:
            import time
            time.sleep(0.3)  # let async write settle
            try:
                ...
                # Build a row dict matching what classify_root_cause expects
                row_dict = {
                    "risky_votes":      active.get("risky_vote_count", 0),
                    "benign_votes":     active.get("benign_vote_count", 0),
                    "reranked":         str(active.get("reranked", False)),
                    "triggered_on":     active.get("triggered_on", ""),
                    "reranker_verdict": active.get("reranker_verdict", ""),
                    "reranker_reason":  active.get("reranker_reason", ""),
                }
                root_cause             = classify_root_cause(row_dict)
                fix_type, fix_desc     = suggest_fix(root_cause, miss_type, user_input)

                # Look up the evaluation_id just written by the async logger
                conn = db.get_connection()
                row = conn.execute(
                    "SELECT id FROM evaluations WHERE prompt_text = ? AND verdict = ? "
                ...
                if row:
                    ...
                    if not already:
                        ...
                        print(f"  [Feedback Footprint] Miss logged — {miss_type} | {root_cause} | {fix_type}")
            except Exception as e:
                print(f"  [Feedback Footprint] Auto-detection error: {e}")

    print_confusion_matrix(results)

    # ---- FEEDBACK FOOTPRINT SUMMARY ----
    wrong_count = sum(1 for r in results if not r["correct"])
    if wrong_count > 0:
        try:
            conn = db.get_connection()
            logged = conn.execute(
                "SELECT COUNT(*) FROM feedback_footprint WHERE fix_applied = 0"
            ).fetchone()[0]
            conn.close()
            print(f"\n[Feedback Footprint] {wrong_count} wrong verdicts this run.")
            print(f"[Feedback Footprint] Total unresolved misses in database: {logged}")
            print(f"[Feedback Footprint] Run: python src/supervisor/failguard_analyzer.py --misses")
        except Exception as e:
            print(f"[Feedback Footprint] Summary error: {e}")
