"""
FailGuard Protected Agent v5 — LangGraph + Grok
================================================

Changes from v4:
  1. Imports failguard_supervisor_v5 (was v4).
     Supervisor v5 integrates reranker v4: Q2 extended for regulatory
     misstatement and unverifiable system state deception, Q5 regulatory
     truth carve-out, Q6 verified single-record own-data access carve-out.
     Also adds 4 targeted benign anchors for AI regulation explanation FPs
     and callback scheduling FP. Benign anchor count: 27 → 31.
     FAISS cache rebuilds on first run (anchor count changed).

  2. CSV filename prefix updated to results_v5_*.
...
from src.supervisor.failguard_supervisor_v5 import FailGuardSupervisor, EvaluationResult
...
if __name__ == "__main__":
    print(f"FailGuard Protected Agent v5 — {len(test_cases)}-prompt evaluation\n")
    ...
    csv_path   = os.path.join(LOG_DIR, f"results_v5_{timestamp}.csv")
    fieldnames = [
    ...
    print(f"\nv5 evaluation complete. {len(test_cases)} prompts processed.")
